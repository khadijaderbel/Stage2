...
export default function EditCategoriePage() {
  ...
  useEffect(() => {
    apiFetch(`/categories/${id}`)
      .then((c) => {
        ...
        applyStyle(validateNom(c.nom), c.nom.trim() === '');
        setLoaded(true);
      })
      .catch((err: any) => setLoadError(err.message || 'Catégorie introuvable.'));
  }, [id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await apiFetch(`/categories/${id}`, { method: 'PUT', body: JSON.stringify({ nom, description }) });
  ...
  return (
    <div className="container-fluid p-0">
      <p className="mb-3" style={{ color: 'var(--text-secondary)' }}>
        Modifiez les informations de <strong style={{ color: 'var(--text-primary)' }}>{nom}</strong>
      </p>
          ...
          <form onSubmit={handleSubmit} id="categorieForm">
              ...
              <div className="col-12">
                <div className="form-group">
                  ...
                  <input
                    ...
                    onBlur={() => applyStyle(validateNom(nom), nom.trim() === '')}
            ...
            <div className="mt-4 d-flex gap-3">
              <button type="submit" className="btn px-4 py-2 fw-semibold" disabled={loading}
                ...
                <i className="fas fa-save me-2"></i> {loading ? 'Enregistrement...' : 'Enregistrer les modifications'}
              </button>
