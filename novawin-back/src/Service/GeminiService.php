...
class GeminiService extends AbstractAiContentGenerator
{
    ...
    public function getNom(): string { return 'Gemini'; }

    protected function appellerModeleTexte(string $prompt, ?array $imageData): string
    {
        $parts = [['text' => $prompt]];
        if ($imageData !== null) {
            $parts[] = ['inline_data' => ['mime_type' => $imageData['mime'], 'data' => base64_encode($imageData['bytes'])]];
        }
        ...
        if (!$texte) throw new \RuntimeException('Réponse Gemini vide ou inattendue.');
